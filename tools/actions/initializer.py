...
def is_initialized(args):
    instance_id = getattr(args, "instance_id", "default")
    lxc_path = tools.config.defaults["lxc"] + f"/waydroid_{instance_id}"
    return os.path.isfile(args.config) and os.path.isdir(tools.config.defaults["rootfs"]) and os.path.isdir(lxc_path)
...
def init(args):
    ...
    status = "STOPPED"
    session = None
    if os.path.exists(tools.config.defaults["lxc"] + f"/waydroid_{session_bus_id}"):
    ...
    if status != "STOPPED":
        if "running_init_in_service" in args:
            ...
            try:
                container = tools.helpers.ipc.DBusContainerService(session_bus_id)
    ...
    class WaydroidInitWindow(Gtk.Window):
        ...
        def on_destroy(self, _):
            if self.initializing:
                try:
                    session_bus_id = getattr(args, "instance_id", "default")
                    tools.helpers.ipc.DBusContainerService(session_bus_id, "/Initializer", "id.waydro.Initializer").Cancel()
                except Exception as e:
        ...
        def run_init(self, systemOta, vendorOta, systemType):
            ...
            try:
                session_bus_id = getattr(args, "instance_id", "default")
                initializer = tools.helpers.ipc.DBusContainerService(session_bus_id, "/Initializer", "id.waydro.Initializer")
