...
def start(args):
    ...
    try:
        _name = dbus.service.BusName(f"id.waydro.Container.{session_bus_id}", dbus.SystemBus(), do_not_queue=True)
...
def prepare_drivers_once(args):
    ...
    instance_id = getattr(args, "instance_id", "default")
    
    # always load binder nodes into args regardless of prepared state
    if instance_id == "default":
        helpers.drivers.loadBinderNodes(args)
    else:
        helpers.drivers.setupInstanceBinderNodes(args)
    
    if instance_id in prepared_drivers:
        return
    
    # rest of the setup that only needs to run once
    ...
    prepared_drivers.append(instance_id)
...
def do_start(args, session):
    ...
    command = [tools.config.tools_src +
               "/data/scripts/waydroid-net.sh", "start", session_bus_id]
    ...
    with open(tools.config.defaults["lxc"] + f"/waydroid_{session_bus_id}/config") as f:
    ...
    instance_id = getattr(args, "instance_id", "default")
    if instance_id != "default":
        # give this instance its own rootfs mountpoint
        instance_rootfs = tools.config.defaults["work"] + f"/rootfs_{instance_id}"
        if not os.path.isdir(instance_rootfs):
            os.makedirs(instance_rootfs)
        tools.config.defaults["rootfs"] = instance_rootfs
        # also update overlay paths
        tools.config.defaults["overlay_rw"] = tools.config.defaults["work"] + f"/overlay_rw_{instance_id}"
        tools.config.defaults["overlay_work"] = tools.config.defaults["work"] + f"/overlay_work_{instance_id}"
        if not os.path.isdir(tools.config.defaults["overlay_rw"]):
            os.makedirs(tools.config.defaults["overlay_rw"])
            os.makedirs(tools.config.defaults["overlay_rw"] + "/system")
            os.makedirs(tools.config.defaults["overlay_rw"] + "/vendor")
        if not os.path.isdir(tools.config.defaults["overlay_work"]):
            os.makedirs(tools.config.defaults["overlay_work"])
            os.makedirs(tools.config.defaults["overlay_work"] + "/system")
            os.makedirs(tools.config.defaults["overlay_work"] + "/vendor")
...
def stop(args, quit_session=True):
    ...
    session_bus_id = getattr(args, "instance_id", "default")
    if session_bus_id != "default":
        tools.config.defaults["rootfs"] = tools.config.defaults["work"] + f"/rootfs_{session_bus_id}"
        tools.config.defaults["overlay_rw"] = tools.config.defaults["work"] + f"/overlay_rw_{session_bus_id}"
        tools.config.defaults["overlay_work"] = tools.config.defaults["work"] + f"/overlay_work_{session_bus_id}"
    ...
    try:
        ...
        # Networking
        command = [tools.config.tools_src +
                   "/data/scripts/waydroid-net.sh", "stop", session_bus_id]
        tools.helpers.run.user(args, command, check=False)
