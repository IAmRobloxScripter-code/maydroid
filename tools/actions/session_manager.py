...
class DbusSessionManager(dbus.service.Object):
    ...
    def Stop(self):
        ...
        stop_container(instance_id=session_bus_id, quit_session=False)
...
def start(args, unlocked_cb=None, background=True):
    session_bus_id = getattr(args, "instance_id", "default")
    try:
        _name = dbus.service.BusName(f"id.waydro.Session.{session_bus_id}", dbus.SessionBus(), do_not_queue=True)
    ...
    if session_bus_id == "default":
        ...
        session["waydroid_user_state"] = session["xdg_data_home"] + f"/waydroid_{session_bus_id}"
    ...
    try:
        session_bus_id = getattr(args, "instance_id", "default")
        tools.helpers.ipc.DBusContainerService(session_bus_id).Start(session)
...
def stop(args):
    session_bus_id = getattr(args, "instance_id", "default") 
    try:
        tools.helpers.ipc.DBusSessionService(session_bus_id).Stop()
    except dbus.DBusException:
        stop_container(instance_id=session_bus_id, quit_session=True)

def stop_container(instance_id = "default", quit_session=False):
    try:
        tools.helpers.ipc.DBusContainerService(instance_id).Stop(quit_session)
    except dbus.DBusException:
        pass
