...
def setupInstanceBinderNodes(args):
    instance_id = getattr(args, "instance_id", "default")
    if instance_id == "default":
        loadBinderNodes(args)
        return

    binderfs_path = f"/dev/binderfs-{instance_id}"
    
    # unmount and remount fresh so no stale context manager
    tools.helpers.run.user(args, ["umount", binderfs_path], check=False)
    tools.helpers.run.user(args, ["mkdir", "-p", binderfs_path], check=False)
    tools.helpers.run.user(args, ["mount", "-t", "binder", "binder", binderfs_path], check=False)

    # use standard node names, not instance-specific
    NRBITS = 8
    TYPEBITS = 8
    SIZEBITS = 14
    NRSHIFT = 0
    ...
    control = f"{binderfs_path}/binder-control"
    ...
    args.BINDER_DRIVER = f"binderfs-{instance_id}/binder"
    args.VNDBINDER_DRIVER = f"binderfs-{instance_id}/vndbinder"
    args.HWBINDER_DRIVER = f"binderfs-{instance_id}/hwbinder"
    
    logging.info(f"Instance {instance_id} binder: {args.BINDER_DRIVER}")
