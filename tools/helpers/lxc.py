...
def generate_nodes_lxc_config(args):
    ...
    if instance_id != "default":
        # mask host binderfs so Android can't use it
        make_entry("tmpfs", "dev/binderfs", "tmpfs", "ro,create=dir 0 0", False)
        # then bind mount instance-specific nodes over /dev/binder etc
        make_entry(f"/dev/binderfs-{instance_id}/binder", "dev/binder",
                options="bind,create=file 0 0", check=False)
        make_entry(f"/dev/binderfs-{instance_id}/vndbinder", "dev/vndbinder",
                options="bind,create=file 0 0", check=False)
        make_entry(f"/dev/binderfs-{instance_id}/hwbinder", "dev/hwbinder",
                options="bind,create=file 0 0", check=False)
    else:
...
def set_lxc_config(args):
    ...
    lxc_path = tools.config.defaults["lxc"] + f"/waydroid_{session_bus_id}"
    ...
    command = ["sed", "-i",
        f"s|/lxc/waydroid/|/lxc/waydroid_{session_bus_id}/|g",
        lxc_path + "/config"]
    tools.helpers.run.user(args, command)

    if session_bus_id != "default":
        instance_rootfs = tools.config.defaults["work"] + f"/rootfs_{session_bus_id}"
        command = ["sed", "-i",
                f"s|lxc.rootfs.path = .*|lxc.rootfs.path = {instance_rootfs}|",
                lxc_path + "/config"]
        tools.helpers.run.user(args, command)

        bridge_id = session_bus_id.replace('_', '').replace('-', '')[:10]
        bridge_name = f"wd{bridge_id}"
        command = ["sed", "-i",
                f"s|lxc.net.0.link = .*|lxc.net.0.link = {bridge_name}|",
                lxc_path + "/config"]
        tools.helpers.run.user(args, command)
...
def status(args):
    session_bus_id = getattr(args, "instance_id", "default")
    command = ["lxc-info", "-P", tools.config.defaults["lxc"], "-n", f"waydroid_{session_bus_id}", "-sH"]
    try:
...
def start(args):
    ...
    command = ["lxc-start", "-P", tools.config.defaults["lxc"],
               "-F", "-n", f"waydroid_{session_bus_id}", "--", "/init"]
...
def stop(args):
    session_bus_id = getattr(args, "instance_id", "default")
    command = ["lxc-stop", "-P",
               tools.config.defaults["lxc"], "-n", f"waydroid_{session_bus_id}", "-k"]
    tools.helpers.run.user(args, command)

def freeze(args):
    session_bus_id = getattr(args, "instance_id", "default")
    command = ["lxc-freeze", "-P", tools.config.defaults["lxc"], "-n", f"waydroid_{session_bus_id}"]
    tools.helpers.run.user(args, command)

def unfreeze(args):
    session_bus_id = getattr(args, "instance_id", "default")
    command = ["lxc-unfreeze", "-P",
               tools.config.defaults["lxc"], "-n", f"waydroid_{session_bus_id}"]
    tools.helpers.run.user(args, command)
...
def android_env_attach_options(args):
    ...
    command = ["lxc-attach", "-P", tools.config.defaults["lxc"],
               "-n", f"waydroid_{session_bus_id}", "--clear-env", "--",
